            ...
            @foreach($employees as $item)
                <tr>
                    ...
                    <td class="text-center">
                        <a href="{{ url('karyawan/'.encrypt($item->id)) }}" class="text-primary mx-1">
                            <i class="fa fa-search"></i>
                        </a>

                        @can('employee-edit')
                        <a href="{{ url('karyawan/'.encrypt($item->id).'/edit') }}" class="text-dark mx-1">
                        ...
                        <form id="delete-item-{{ md5($item->id) }}"
                            action="{{ url('karyawan/'.encrypt($item->id)) }}"
                            method="post"
                            style="display: none;"
                        >    
                            @csrf
